...
public class MainActivity extends Activity {
  ...
  boolean isPromptDeckImportV17(String raw){return raw!=null&&raw.trim().startsWith("PROMPTDECK_IMPORT_V1");}
  ...
  void showBulkPaste(){
    ...
    AlertDialog d=new AlertDialog.Builder(this).setTitle("Add a prompt").setView(box).setNegativeButton("Cancel",null).setNeutralButton("Create from images",null).setPositiveButton("Add prompt",null).create();
  ...
  int capabilityCategoryBoostV8(Cmd c,String cap){
  ...
  void renderCapabilityChooserV8(LinearLayout target,String domain,String goal,String forcedMode){
    ...
    String[] ids=CapabilityRouter.capabilities(domain,mode);for(int i=0;i<ids.length;i+=2){LinearLayout row=hbox();final String left=ids[i];View a=capabilityTileV8(left,()->{askCapability=left;renderCapabilityRecommendationAsyncV20(target,goal,left);});LinearLayout.LayoutParams ap=new LinearLayout.LayoutParams(0,-2,1);ap.setMargins(0,0,i+1<ids.length?dp(6):0,dp(6));row.addView(a,ap);if(i+1<ids.length){final String right=ids[i+1];View b=capabilityTileV8(right,()->{askCapability=right;renderCapabilityRecommendationAsyncV20(target,goal,right);});LinearLayout.LayoutParams bp=new LinearLayout.LayoutParams(0,-2,1);bp.setMargins(0,0,0,dp(6));row.addView(b,bp);}target.addView(row);}
  ...
  boolean payloadSafe(String text){return text==null||text.getBytes(StandardCharsets.UTF_8).length<=600000;}
  ...
  void send(){if(finalPrompt==null)return;String text=finalPrompt.getText().toString();if(!payloadSafe(text)){toast("Prompt is too large to send safely. Remove a prompt from the stack or shorten the request.");return;}Intent i=new Intent(Intent.ACTION_SEND);i.setType("text/plain");i.putExtra(Intent.EXTRA_TEXT,text);i.setPackage("com.openai.chatgpt");try{startActivity(i);}catch(Exception e){i.setPackage(null);startActivity(Intent.createChooser(i,"Send prompt"));}}
