...
def load():
    ...
    for src,fn in SOURCES:
        ...
        for x in json.load(open(p,encoding='utf-8')):
            ...
            out.append({'gid':gid,'source':src,'title':title,'category':cat(x.get('category')),'subcategory':x.get('subcategory') or '', 'description':(x.get('description') or '').strip(), 'prompt':prompt.strip()})
            gid+=1
    return out
...
def main():
    ...
    for category in sorted(bycat):
        ...
        packet_size=75; safe=re.sub(r'[^a-z0-9]+','_',category.lower()).strip('_')
        for idx in range(0,len(xs),packet_size):
            chunk=xs[idx:idx+packet_size]; fn=f'{safe}_{idx//packet_size+1:02d}.md'
            lines=[f'# Manual Review Packet — {category} — {idx//packet_size+1}','',f'Entries: {len(chunk)}','']
            for x in chunk:
                preview=clean(x['prompt'])[:650]; desc=clean(x['description'])[:220]
            ...
            manifest.append({'file':fn,'category':category,'count':len(chunk)})
